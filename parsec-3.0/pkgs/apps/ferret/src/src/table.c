...
// RSDG:new arg for configurable var
int cass_table_query (cass_table_t *table,
                     cass_query_t *query, cass_result_t *result, int maxItr)
{
	if (table->opr->query != NULL) {
		return table->opr->query(table, query, result, maxItr);
	}
	else
	{
                           ...
                           cass_result_t **results)
{
	...
	else
	{
		...
		for (i = 0; i < count; i++)
		{
			//dummy 0 added by RSDG
			table->opr->query(table, queries[i], results[i],0);
		}
	}
	return 0;
}
