...
float
emd(signature_t *Signature1, signature_t *Signature2, float (*Dist)(cass_size_t, feature_t, feature_t, void *), cass_size_t dim, void *param, flow_t *Flow, int*FlowSize, int max_iterations)
{
  ...
  if (state->n1 > 1 && state->n2 > 1)  /* IF _n1 = 1 OR _n2 = 1 THEN WE ARE DONE */
	...
	//	max_iterations);
	//fprintf(stdout, "maximum number of iterations has been reached\n");
	}
    }
