...
#define MAX_PROB	100
static int raw_query(cass_table_t *table, cass_query_t *query, cass_result_t *result, int max_iteration)
{
	struct raw_private *priv = (struct raw_private *)table->__private;
	...
	if (query->topk > 0)
	{
		...
		if (query->candidate != NULL)
		{
			if (query->candidate->flags & CASS_RESULT_LIST)
			{
				ARRAY_BEGIN_FOREACH(query->candidate->u.list, cass_list_entry_t cand)
				{
					...
					entry.id = cand.id;
					entry.dist = vecset_dist->__class->dist(ds, entry.id, query->dataset, query->vecset_id, vec_dist, vecset_dist, max_iteration);
		...
		else
		{
			for (i = 0; i < ds->num_vecset; i++)
			{
				...
				cass_list_entry_t entry;
				fprintf(stdout, "Im here at 202\n");
				entry.dist = vecset_dist->__class->dist(ds, i, query->dataset, query->vecset_id, vec_dist, vecset_dist, max_iteration);
				entry.id = i;
				TOPK_INSERT_MIN(result->u.list.data, dist, query->topk, entry);
			}
		}
